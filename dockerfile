...
RUN apk --no-cache add \
    ...
    && pip3 install --break-system-packages requests schedule python-dateutil
...
# Создаем файл крона
RUN echo "20 4 * * * root /usr/bin/python3 /opt/app/getfromsd.py > /dev/stdout" > /etc/periodic/daily/mycronjob
RUN echo "21 4 * * * root /usr/bin/python3 /opt/app/getfronjson.py > /dev/stdout" >> /etc/periodic/daily/mycronjob
